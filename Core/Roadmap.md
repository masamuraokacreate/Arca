...
- **Sprint 10: Finance（家計・支出管理）モジュール** 【完了】
  - 支出特化の独立ビュー、レシート品目内訳（1対Nリレーショナル）、クレジットカード明細CSV（三井住友・dカード・イオン・View等）自動突合エンジン、月次支出分析・日別推移可視化、Google Drive完全バックアップ統合。
- **Sprint 10.5: Finance（カメラ・レシートOCR解析 ＆ 親子決済）拡張** 【完了】
  - iPhone標準カメラ連携（ワンタップ起動）、Gemini Vision マルチモーダルOCR解析（印字文字・金額そのまま忠実抽出）、外税・値引きの独立品目化による親子決済合計完全整合、プレビュー確認・編集モーダル。
- **Sprint 11: テーマ切り替え機能（ベースカラー変更 ＆ ダークモード対応）** 【完了】
  - CSSカスタムプロパティ（CSS変数）によるマルチテーマアーキテクチャ（アイボリー、ディープスペース、ウォームサンド、ミッドナイトセージ ＆ OSシステム連動）、ThemeContext/useThemeフック、初期チラつき防止（FOUC防止）、Apple HIG準拠の外観設定モーダル（ThemeModal）。
